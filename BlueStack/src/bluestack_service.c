...
uint32_t BlueStack_ReadCharAdd(BlueStack_Characteristic_t* c, uint8_t service_id) {
    ...
    return sd_ble_gatts_characteristic_add(_services[service_id].service_handle, &char_md,
                                               &attr_char_value,
                                               &c->handle);
...
/*
    err_code = sd_ble_uuid_vs_add(&service->service_uuid_base, &service->service_uuid_type);
    CHECK_ERROR(err_code);

    ble_uuid.type = service->service_uuid_type;
    ble_uuid.uuid = service->service_uuid;

    err_code = sd_ble_gatts_service_add(BLE_GATTS_SRVC_TYPE_PRIMARY, &ble_uuid, &service->service_handle);
    CHECK_ERROR(err_code);

    char_aux = service->chars;
    count = service->chars_count;
    while(count){
        if(char_aux->type & BLUESTACK_CHAR_TYPE_WR){
            BlueStack_WriteCharAdd(char_aux, service->service_handle);
        }
        else
        if(char_aux->type & (BLUESTACK_CHAR_TYPE_RD | BLUESTACK_CHAR_TYPE_RD)){
            BlueStack_ReadCharAdd(char_aux, service->service_handle);
        }
        char_aux++;
        count--;
    }
